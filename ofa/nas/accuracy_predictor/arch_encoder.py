...
class OFAProxylessNASNetsEncoder:
    def __init__(
        self,
        image_size_list=None,
        ks_list=None,
        ...
        self.image_size_list = [224] if image_size_list is None else image_size_list
        self.ks_list = [3, 5, 7] if ks_list is None else ks_list
        self.expand_list = [3, 4, 6] if expand_list is None else expand_list
        self.depth_list = [2, 3, 4] if depth_list is None else depth_list
    ...
    @property
    def max_n_blocks(self):
        return len(self.depth_list) * max(self.depth_list)
        ...
        if target == "r":
            ...
            for i in range(self.max_n_blocks):
                ...
                for choice in choices:
                    target_dict["val2id"][i][choice] = self.n_dim
                    target_dict["id2val"][i][self.n_dim] = choice
                    self.n_dim += 1
                target_dict["R"].append(self.n_dim)
    ...
    def arch2feature(self, arch_dict):
        ...
        for i in range(self.max_n_blocks):
            feature[self.k_info["val2id"][i][ks[i]]] = 1
            feature[self.e_info["val2id"][i][e[i]]] = 1
            feature[self.w_info["val2id"][i][w[i]]] = 1
        feature[self.r_info["val2id"][r]] = 1
        return feature
    ...
    def feature2arch(self, feature):
        ...
        assert img_sz in self.image_size_list
        arch_dict = {"ks": [], "e": [], "w": [], "image_size": img_sz}

        for i in range(self.max_n_blocks):
            arch_dict["ks"].append(
                self.k_info["id2val"][i][
                    int(np.argmax(feature[self.k_info["L"][i] : self.k_info["R"][i]]))
                    + self.k_info["L"][i]
                ]
            )
            arch_dict["e"].append(
                self.e_info["id2val"][i][
                    int(np.argmax(feature[self.e_info["L"][i] : self.e_info["R"][i]]))
                    + self.e_info["L"][i]
                ]
            )
            arch_dict["w"].append(
                self.w_info["id2val"][i][
                    int(np.argmax(feature[self.w_info["L"][i] : self.w_info["R"][i]]))
                    + self.w_info["L"][i]
                ]
            )
        return arch_dict
    ...
    def random_sample_arch(self):
        return {
            "ks": random.choices(self.ks_list, k=self.max_n_blocks),
            "e": random.choices(self.expand_list, k=self.max_n_blocks),
            "w": random.choices(list(range(len(self.width_mult_list))), k=self.max_n_blocks),
            "image_size": random.choice(self.image_size_list),
        }
